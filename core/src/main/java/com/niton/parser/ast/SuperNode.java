...
public class SuperNode extends AstNode {
    ...
    public ReducedNode reduce(@NonNull String name) {
        ...
        if (subNodes.isEmpty() && naming.isEmpty()) {
            return ReducedNode.leaf(name, joinTokens());
        }
        if(subNodes.size() == 1 && naming.size() == 1){
            var node = subNodes.get(0);
            if(node.isLeaf()){
                return ReducedNode.leaf(name,node.getValue());
            }else{
                return ReducedNode.node(name,node.getChildren());
            }
        }
